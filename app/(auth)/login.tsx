...
export default function Login() {
  ...
  const [tempToken, setTempToken] = useState<string | null>(null);   // vaqtinchalik token (ism kiritish uchun)
  ...
  async function verifyOTP(code: string) {
    ...
    try {
      ...
      // Agar PIN mavjud bo‘lmasa VA ism kiritilmagan bo‘lsa → ism so‘raladi
      if (!res.hasPin && !name.trim()) {
        setTempToken(res.token);        // tokenni saqlaymiz
        setLoading(false);
        setStep('name');
        return;
      }
      // Aks holda to‘liq login
  ...
  // Ismni yuborish (vaqtinchalik token orqali)
  async function submitName() {
    if (!name.trim() || !tempToken) return;
    setLoading(true);
    try {
      // Agar API da ismni yangilash uchun alohida endpoint bo‘lsa:
      await api.updateUserName(tempToken, name.trim());
      // Keyin login qilamiz (token allaqachon mavjud)
      // User ma’lumotlarini olish uchun qo‘shimcha so‘rov kerak bo‘lishi mumkin
      // Oddiy holat: tokenni saqlab, PIN yaratishga o‘tamiz
      await login(tempToken, { phone: fullPhone, name: name.trim() }, false);
                      ...
                      {otp.map((d, i) => (
                        <TextInput
                          ...
                          const s = StyleSheet.create({
                            ...
                            phoneIn: { flex: 1, paddingHorizontal: 12, fontSize: 16, fontWeight: '600', color: C.t1, letterSpacing: 0.
                                5 },
