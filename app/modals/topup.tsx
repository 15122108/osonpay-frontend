...
export default function TopUp() {
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (res.status === 'completed') {
            ...
            await refresh();
            setStep('success');
          } else if (res.status === 'failed' || res.status === 'cancelled' || res.status === 'declined') {
            clearInterval(pollRef.current);
            setStep('failed');
          }
        } catch {}
      }, 3000); // 3 soniyada bir tekshirish
    }
  ...
  async function startTopup() {
    ...
    if (!amt || amt < 1000) {
      Alert.alert(t('error'), 'Minimum 1,000 UZS');
      return;
    }
    setLoading(true);
    try {
  ...
  function handleWebViewNav(navState: any) {
    ...
    // Qaytish URL — osonpay://payment?ref=xxx
    if (url.startsWith('osonpay://') || url.includes('/api/payments/return')) {
      // To'lov sahifasidan qaytdi — polling davom etadi
    }
  }
    ...
    return (
      <SafeAreaView style={s.root}>
        <View style={s.resultWrap}>
          ...
          <Text style={s.resultSub}>{formatMoney(Number(amount))} UZS hisobingizga o'tkazildi</Text>
          <TouchableOpacity style={s.resultBtn} onPress={() => router.back()}>
            <LinearGradient colors={C.gSuccess} style={s.resultBtnGrad}>
    ...
    return (
      <SafeAreaView style={s.root}>
        <View style={s.resultWrap}>
          ...
          <Text style={s.resultSub}>Karta ma'lumotlarini tekshiring yoki qayta urinib ko'ring</Text>
          <TouchableOpacity style={s.resultBtn} onPress={() => setStep('amount')}>
            <LinearGradient colors={C.gOrange} style={s.resultBtnGrad}>
