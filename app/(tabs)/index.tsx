...
const SAVED_PAYMENTS = [
  ...
  { id: '3', name: "HUDUDGAZTA'...",    sub: "55 799.51...", icon: '🔥', badge: '1%' },
];
      ...
      >
        ...
        <View style={s.header}>
          <View style={s.headerLeft}>
            <TouchableOpacity onPress={() => router.push('/modals/profile' as any)}>
        ...
        <View style={s.actionsRow}>
          {[
            { label: t('send'),    icon: '↑', grad: C.gBrand,    route: '/(tabs)/send' },
            { label: t('receive'), icon: '↓', grad: C.gSuccess,  route: '/modals/receive' },
            { label: t('topup'),   icon: '+', grad: C.gOrange,   route: '/modals/topup' },
            { label: t('payment'), icon: '⊙', grad: C.gPrimary,  route: '/(tabs)/payments' },
          ].map(a => (
            <TouchableOpacity key={a.label} style={s.actionWrap} onPress={() => router.push(a.route as any)} activeOpacity={0.8}>
          ...
          <TouchableOpacity style={s.keshbekWrap} activeOpacity={0.85}>
            <LinearGradient colors={C.gBrand} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.keshbekGrad}>
              <Text style={s.keshbekTxt}>
                🎁 {formatMoney((stats.cashback || 0))} so'm keshbekingiz bor {'›'}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
        )}
        ...
        <View style={s.sec}>
          ...
          <TouchableOpacity style={s.transferBox} onPress={() => router.push('/(tabs)/send' as any)} activeOpacity={0.7}>
        ...
        <View style={s.sec}>
          <View style={s.secHead}>
            ...
            <TouchableOpacity onPress={() => router.push('/(tabs)/payments' as any)}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: S.sm }}>
            {SAVED_PAYMENTS.map(p => (
              <TouchableOpacity key={p.id} style={s.savedCard} activeOpacity={0.7}>
                ...
                <Text style={s.savedSub}>{p.sub}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
